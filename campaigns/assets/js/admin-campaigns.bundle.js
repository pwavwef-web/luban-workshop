(() => {
  // src/admin-campaigns.js
  var smsCampaignState = {
    initialized: false,
    loading: false,
    scheduleMode: "now",
    counts: {
  ...
  function getManualSmsNumbers() {
    ...
    const numbers = raw.split(/[,\n;]+/).map((value) => formatGhanaPhone(value.trim())).filter(Boolean);
  ...
  function setSmsScheduleMode(mode) {
    ...
    pill.className = smsCampaignState.scheduleMode === "scheduled" ? "px-2 py-1 text-xs font-bold rounded-full bg-blue-50 text-blue-700 border border-blue-100" : "px-2 py-1 text-xs font-bold rounded-full bg-green-50 text-green-700 border border-green-100";
    updateSmsCampaignSummary();
  }
  ...
  function renderSmsCampaignHistory() {
    ...
    container.innerHTML = campaigns.map((campaign) => {
      const scheduledLine = campaign.scheduled ? `<p class="text-xs text-blue-700 mt-1">Scheduled: ${escapeHtml(campaign.scheduleLocal || campaign.providerSchedule || campaign.scheduleAt || "")}</p>` : "";
      const contactLine = campaign.contactSyncCount || campaign.contactSyncFailedCount ? `<p class="text-xs text-stone-500 mt-1">Contacts: ${Number(campaign.contactSyncCount || 0)} synced, ${Number(campaign.contactSyncFailedCount || 0)} failed</p>` : "";
      return `
  ...
  async function refreshSmsCampaigns() {
    ...
    try {
      ...
      getSmsCampaignElement("sms-campaign-history").innerHTML = `<p class="p-6 text-sm text-red-600">${escapeHtml(error.message || "Could not load SMS campaigns.")}</p>`;
    } finally {
  ...
  function buildSmsCampaignPayload() {
    ...
    return {
      ...
      manualRecipients: getSmsCampaignElement("sms-campaign-audience").value === "manual" ? getSmsCampaignElement("sms-campaign-manual").value : "",
      scheduleMode: smsCampaignState.scheduleMode,
      scheduleAt: smsCampaignState.scheduleMode === "scheduled" ? scheduleAt : "",
      scheduleTimezone: Intl.DateTimeFormat().resolvedOptions().timeZone || "",
      scheduleTimezoneOffsetMinutes: (/* @__PURE__ */ new Date()).getTimezoneOffset(),
      syncContacts,
