(() => {
  ...
  var getExperimentalSetting = (name5) => {
    var _a;
    return (_a = getDefaults()) == null ? void 0 : _a[`_${name5}`];
  };
  var Deferred = class {
    constructor() {
      this.reject = () => {
...
${this.customData.serverResponse}`;
  ...
  // src/firebase-modular-bridge.js
  function ensureApp() {
    if (!getApps().length) {
  ...
  var docRefRegistry = /* @__PURE__ */ new WeakMap();
    ...
    return {
      ...
      runTransaction(updateFunction) {
        return runTransaction(
          db,
          (transaction) => updateFunction(createTransactionApi(transaction))
        );
      }
    };
  }
  function createStorageRefApi(refObj) {
    return {
      ...
      child(path) {
        return createStorageRefApi(ref(refObj, path));
      },
  ...
  function getStorageApi() {
    ...
    return {
      ref(path) {
        return createStorageRefApi(ref(storage, path || ""));
      }
    };
  }
  var firebase = {
