(() => {
  // src/admin-special-menus.js
  var specialMenusCache = [];
  var editingSpecialMenuId = null;
  var specialMenuItems = [];
  function getAdminDb() {
                ...
                <div class="flex items-start justify-between gap-3 border border-stone-200 rounded-lg p-3 bg-white">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            ${item.price !== null && item.price !== void 0 && item.price !== "" ? `<span class="text-xs font-bold text-red-700">${escapeHtml(formatCurrency(item.price))}</span>` : ""}
                        </div>
  ...
  function listenToSpecialMenus() {
    getAdminDb().collection("specialMenus").onSnapshot((snapshot) => {
      ...
      document.getElementById("special-menus-list").innerHTML = '<p class="px-6 py-8 text-center text-red-600 italic">Failed to load special menus.</p>';
  ...
  function renderSpecialMenusList() {
    ...
    list.innerHTML = [...specialMenusCache].sort((a, b) => getSpecialMenuTimestamp(b.data) - getSpecialMenuTimestamp(a.data)).map((item) => {
      const data = item.data;
      const url = getSpecialMenuUrl(item.id);
      const active = data.active === true;
      const statusBadge = active ? '<span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Active</span>' : '<span class="px-2 py-1 text-xs font-semibold rounded-full bg-stone-100 text-stone-700">Hidden</span>';
      return `
