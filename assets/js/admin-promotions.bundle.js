(() => {
  // src/admin-promotions.js
  var promotionsCache = [];
  var editingPromotionId = null;
  var promotionSelectedItems = [];
  function getAdminDb() {
  ...
  function getCurrentDishPrice(dish) {
    return typeof window.getDishCurrentPrice === "function" ? window.getDishCurrentPrice(dish) : Number(dish.price || 0);
  }
  ...
  function syncPromotionSelectionFromInputs() {
    ...
    document.querySelectorAll("[data-promotion-dish-id]").forEach((checkbox) => {
      ...
      if (!dish) return;
      const priceInput = Array.from(document.querySelectorAll("[data-promotion-price-id]")).find((input) => input.dataset.promotionPriceId === dish.id);
  ...
  function getPromotionPricingLine(data) {
    ...
    if (data.discountType === "bundle" && data.bundlePrice !== void 0 && data.bundlePrice !== null && data.bundlePrice !== "") {
    ...
    const priced = items.filter((item) => item.promoPrice !== null && item.promoPrice !== void 0 && item.promoPrice !== "");
  ...
  function listenToPromotions() {
    getAdminDb().collection("promotions").onSnapshot((snapshot) => {
      ...
      document.getElementById("promotions-table").innerHTML = '<tr><td colspan="4" class="px-6 py-8 text-center text-red-600 italic">Failed to load promotions and deals.</td></tr>';
  ...
  function renderPromotionsTable() {
    ...
    const rows = sortPromotions([...promotionsCache]).map((item) => {
      ...
      const statusBadge = visible ? '<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Visible</span>' : '<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-stone-100 text-stone-700">Hidden</span>';
  ...
  function showPromotionFeedback(message, isError) {
    ...
    setTimeout(() => feedback.classList.add("hidden"), 5e3);
  }
  ...
  function openPromotionEditor(id) {
    ...
    promotionSelectedItems = Array.isArray(item.data.items) ? item.data.items.map((promoItem) => ({
      ...
      promoPrice: promoItem.promoPrice === void 0 ? null : promoItem.promoPrice,
  ...
  async function savePromotion(event) {
    ...
    const normalizedItems = promotionSelectedItems.map((item) => {
      ...
      const promoPrice = discountType === "percent" ? Math.round(basePrice * (100 - discountPercent)) / 100 : Number.isFinite(Number(item.promoPrice)) ? Number(item.promoPrice) : null;
      return {
        id: item.id,
        name: item.name,
