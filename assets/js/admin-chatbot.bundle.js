(() => {
  // src/admin-chatbot.js
  var chatbotKnowledgeCache = [];
  var editingChatbotKnowledgeId = null;
  function getAdminDb() {
  ...
  function listenToChatbotKnowledge() {
    getAdminDb().collection("chatbotKnowledge").onSnapshot((snapshot) => {
      ...
      document.getElementById("chatbot-knowledge-table").innerHTML = '<tr><td colspan="4" class="px-6 py-8 text-center text-red-600 italic">Failed to load chatbot facts.</td></tr>';
  ...
  function renderChatbotKnowledgeTable() {
    ...
    const rows = sortChatbotKnowledgeItems([...chatbotKnowledgeCache]).map((item) => {
      ...
      const statusBadge = status === "active" ? '<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Active</span>' : '<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-stone-100 text-stone-700">Archived</span>';
