...
if (adminDateEl) {
    adminDateEl.textContent = new Date().toLocaleDateString(undefined, {
        weekday: 'short',
        month: 'short',
        day: 'numeric'
    });
...
// --- Admin Phone Capture Modal ---
// Replaces the old window.prompt() flow for collecting a missing admin phone
// number with a proper in-dashboard modal (see #admin-phone-modal in admin.html).
let adminPhoneUser = null;
...
async function submitAdminPhone() {
    ...
    if (!adminPhoneUser) { closeAdminPhoneModal(); return; }
    ...
    try {
        ...
        if (saveBtn) { saveBtn.disabled = false; saveBtn.textContent = 'Save number'; }
    }
}
...
const ADMIN_MODULE_ENTRYPOINTS = [
    'listenToMenu',
